...
abumarkub.ui.createMIDIProgramSelector = function(parentDiv,callback){
    ...
    progChange.addEventListener("change", function(e) {
        var programId = progChange.options[progChange.selectedIndex].id;
        callback(programId);
        //midiBridge.sendMidiEvent(midiBridge.PROGRAM_CHANGE, 0, progChange.options[progChange.selectedIndex].id, 0);
    },false);
    
    parentDiv.appendChild(progChange);
    
    return {
        selectOption : function(index){
            progChange.selectedIndex = index;
        }
    }
}